...
export function AdditionalTutoringPage({
  ...
  const narrative = getTutoringNarrative({
    key: r.page.url,
    region: `${r.region.sido} ${r.region.sigungu} ${r.region.eupmyeondong}`,
    school: r.school.name,
    grade: r.page.grade,
    subject: r.page.subject,
    concern: c.concern,
    method: c.studyMethod,
  });
  ...
  return <><Header /><main className="bg-white text-black">
    ...
    <article>
      ...
      <section className="py-20 lg:py-28"><div className="mx-auto max-w-6xl px-5 lg:px-8">
        ...
        <p className="mt-6 max-w-4xl text-lg leading-8 text-black/65">{narrative.diagnosis}</p>
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          <section className="rounded-[28px] bg-[#faf8ff] p-7"><h3 className="text-2xl font-black">학생의 학습 고민</h3><p className="mt-4 leading-8 text-black/62">{c.concern} {narrative.habit}</p></section>
          <section className="rounded-[28px] bg-[#16072f] p-7 text-white"><h3 className="text-2xl font-black">대표적인 학부모 상담 유형</h3><p className="mt-4 leading-8 text-white/70">{c.parentConcern} {narrative.parentCommunication}</p></section>
        </div>
      </div></section>
      <section className="bg-[#faf8ff] py-20 lg:py-28"><div className="mx-auto grid max-w-6xl gap-10 px-5 lg:grid-cols-2 lg:px-8">
        ...
        <div className="space-y-6 text-lg leading-8 text-black/65"><p>{c.schoolAnalysis} {narrative.schoolContext}</p><p>{c.examCaveat}</p><p>{c.exam} {narrative.subject}</p><p className="rounded-2xl bg-white p-6 font-bold text-black">{c.studyMethod} {narrative.selfStudy}</p><h3 className="text-2xl font-black text-black">학습 예시: {c.theme}</h3><p>{c.workedExample}</p></div>
      </div></section>
      <section className="py-20 lg:py-28"><div className="mx-auto max-w-6xl px-5 lg:px-8">
        ...
        <p className="mt-6 max-w-4xl text-lg leading-8 text-black/65">{c.difficultUnit} {narrative.teacherFit}</p>
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          <section className="rounded-[28px] border border-black/8 p-7"><p className="text-sm font-black text-[#6736C8]">01 · 현재 상태 진단</p><h3 className="mt-3 text-2xl font-black">어려운 지점 확인</h3><p className="mt-4 leading-8 text-black/62">{c.concern} {narrative.habit}</p></section>
          <section className="rounded-[28px] border border-black/8 p-7"><p className="text-sm font-black text-[#6736C8]">02 · 내신 대비</p><h3 className="mt-3 text-2xl font-black">실제 범위에 맞춘 준비</h3><p className="mt-4 leading-8 text-black/62">{c.exam} {narrative.schoolContext}</p></section>
          <section className="rounded-[28px] bg-[#f1ebff] p-7"><p className="text-sm font-black text-[#6736C8]">03 · 부족한 부분 보완</p><h3 className="mt-3 text-2xl font-black">혼자 해보는 단계</h3><p className="mt-4 leading-8 text-black/62">{c.studyMethod} {narrative.selfStudy}</p></section>
        </div>
        <div className="mt-10 rounded-[30px] bg-[#16072f] p-8 text-white sm:p-10"><h3 className="text-2xl font-black sm:text-3xl">{c.theme} 중심의 1:1 수업 방향</h3><p className="mt-4 max-w-4xl leading-8 text-white/72">{c.lesson} {narrative.teacherFit}</p><OpenConsultationButton className={buttonVariants({ size: "lg", className: "mt-7 bg-white text-black hover:bg-white/90" })}>내 상황 무료로 상담하기</OpenConsultationButton></div>
      </div></section>
      <section className="bg-[#faf8ff] py-20 lg:py-28"><div className="mx-auto max-w-6xl px-5 lg:px-8">
        ...
        <p className="mt-6 max-w-4xl text-lg leading-8 text-black/65">{narrative.consultation}</p>
        <ol className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{[["상담 신청", `${c.parentConcern} ${narrative.parentCommunication}`], ["학습 기록 확인", c.difficultUnit], ["보완 순서 제안", `${c.studyMethod} ${narrative.selfStudy}`], ["가능한 수업 안내", `${narrative.teacherFit} ${r.school.name}의 진도와 ${c.theme} 목표를 확인하고 일정·방문 또는 화상·비용을 안내합니다.`]].map(([title, body], i) => <li key={title} className="rounded-[26px] bg-white p-7 shadow-sm shadow-black/5"><span className="text-sm font-black text-[#6736C8]">STEP {i + 1}</span><h3 className="mt-3 text-xl font-black">{title}</h3><p className="mt-3 leading-7 text-black/60">{body}</p></li>)}</ol>
      </div></section>
